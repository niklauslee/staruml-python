...
class PyCallable extends PyObject {
    ...
    _getParameters() {
        ...
        if (this._ctx.args) {
            var count = this._ctx.args.arguments.args.length - this._ctx.args.arguments.defaults.length

            this._ctx.args.arguments.args.forEach((element, index) => {
                var parameter = new PyParameter(element)
                if (index - count >= 0) {
                    parameter.default = this._ctx.args.arguments.defaults[index - count].Constant.value
                }
...
class PyFunction extends PyCallable {
    ...
    _isStatic() {
        return Boolean(this._ctx?.decorator_list?.find(decorator => decorator?.Name?.id == 'staticmethod'))
    }
    ...
    getVisibility() {
        return this.isConstructor ? type.UMLModelElement.VK_PUBLIC : PyCallable.prototype.getVisibility.call(this)
    }
}
...
class PyPackage {
    ...
    _translatePackagesAndModules(pth) {
        ...
        if (stat.isFile()) {
            ...
            if (ext === '.py') {
                try {
                    var result = execSync(`${this._options.pythonPath} "${path.join(__dirname, 'ast2json.py')}" "${pth}"`);
